...
- **Lecture Wings** : `DaemonFileRepository::getContent($path, 512_000)` — sur le panel Pterodactyl courant, Wings renvoie le fichier entier puis une exception si la taille dépasse **512 Ko** (pas de lecture partielle « tête de fichier » dans ce flux).
...
- **Parser** : `PmcpVersionLogParser::parse()` — pur, sans I/O. Avant les regex : retrait **BOM UTF-8** et **séquences ANSI** (CSI / OSC). Loaders reconnus : **paper**, **purpur**, **folia**, **pufferfish**, **leaf** (ligne « … version » + `(MC: …)` ou `(Implementing API version …)`), **spigot** (CraftBukkit), **neoforge**, **forge**, **quilt**, **fabric**, **bedrock**, **vanilla**. Les forks Paper-like sans `(MC: …)` sur la même ligne utilisent la version MC extraite de **Implementing API version** (ex. Purpur / Folia 1.21+). Vanilla : motif strict puis repli pour une ligne qui commence par `Starting minecraft server version` sans préfixe `[thread/INFO]:` habituel.
